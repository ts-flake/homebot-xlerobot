...
class HumanFollowApp:
    ...
    def initialize(self) -> bool:
        ...
        try:
            ...
            # 4. 初始化控制器
            logger.info("初始化控制器...")
            # 从配置获取相机分辨率 (人体跟随用 head 相机, 缺省取第一个)
            from configs.config import get_config
            _cams = get_config().cameras
            cam_config = _cams.get("head") or next(iter(_cams.values()))
            ...
            # 5. 初始化底盘客户端
            logger.info("连接底盘服务...")
            from software.src.homebot.services.motion_service.clients import ChassisClient
    ...
    def _visualize(self, frame: np.ndarray, target, detections) -> np.ndarray:
        ...
        info_lines = [
            ...
            f"Velocity: vx={self.current_velocity.vx:+.2f}, vtheta={self.current_velocity.vtheta:+.2f}",
