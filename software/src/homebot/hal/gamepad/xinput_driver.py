...
class XInputDriver:
    ...
    def __init__(self, controller_index: int = 0):
        ...
        self._state: Optional[ControllerState] = None  # update() 缓存, 供统一 Gamepad 接口读取
    ...
    # ==================== 统一 Gamepad 接口 (适配 base.Gamepad 契约) ====================
    # 把 XInput 原生 API 适配到与 SDLDriver 相同的语义接口, 使 keymap 引擎 / app 与后端无关.

    # 语义按键名 → XInput 位标志 (lt/rt 走扳机阈值, 单独处理)
    _SEMANTIC_FLAG = {
        "a": ButtonFlags.A, "b": ButtonFlags.B, "x": ButtonFlags.X, "y": ButtonFlags.Y,
        "lb": ButtonFlags.LEFT_SHOULDER, "rb": ButtonFlags.RIGHT_SHOULDER,
        "start": ButtonFlags.START, "back": ButtonFlags.BACK, "logo": ButtonFlags.GUIDE,
        "ls": ButtonFlags.LEFT_THUMB, "rs": ButtonFlags.RIGHT_THUMB,
    }

    def connect(self) -> None:
        """DLL 已在 __init__ 加载; 这里仅刷新一次状态以备读取."""
        self.update()
    ...
    def _cur(self) -> ControllerState:
        if self._state is None:
            self._state = self.get_state()
        return self._state

    def get_button(self, name: str) -> bool:
        """语义按键是否按下 (a/b/x/y, lb/rb, lt/rt, start/back/logo, ls/rs)."""
        st = self._cur()
        if name == "lt":
            return st.left_trigger > 0.5
        if name == "rt":
            return st.right_trigger > 0.5
        flag = self._SEMANTIC_FLAG.get(name)
        if flag is None:
            raise ValueError(f"Invalid button: {name}")
        return st.is_pressed(flag)

    def get_left_stick(self) -> tuple[float, float]:
        """(x, y); y 正=下 (翻转 XInput 的正上约定以对齐 SDL 契约)."""
        st = self._cur()
        return (st.left_stick.x, -st.left_stick.y)

    def get_right_stick(self) -> tuple[float, float]:
        st = self._cur()
        return (st.right_stick.x, -st.right_stick.y)

    def get_dpad(self) -> tuple[int, int]:
        """(x, y); x 正=右, y 正=上."""
        st = self._cur()
        x = int(st.is_pressed(ButtonFlags.DPAD_RIGHT)) - int(st.is_pressed(ButtonFlags.DPAD_LEFT))
        y = int(st.is_pressed(ButtonFlags.DPAD_UP)) - int(st.is_pressed(ButtonFlags.DPAD_DOWN))
        return (x, y)
