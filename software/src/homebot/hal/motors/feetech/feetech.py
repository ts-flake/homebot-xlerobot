...
#
# Vendored from lerobot.motors.feetech.feetech. Imports rerouted to local modules.
...
from .._utils import _feetech_sdk_available, require_package
...
class OperatingMode(Enum):
    # position servo mode
    POSITION = 0
    # constant speed mode, controlled by parameter 0x2e; highest bit 15 is the direction bit
    VELOCITY = 1
    # PWM open-loop speed regulation, controlled by parameter 0x2c run-time; bit11 is the direction bit
    PWM = 2
    # step servo mode, step count via parameter 0x2a; highest bit 15 is the direction bit
    STEP = 3
...
def patch_setPacketTimeout(self, packet_length):  # noqa: N802
    """
    Patch PortHandler.setPacketTimeout to use correct packet-length-aware timeouts.

    Fixes a bug present in the PyPI `feetech-servo-sdk` package
    (https://gitee.com/ftservo/SCServoSDK/issues/IBY2S6). The fix exists upstream on the
    official Feetech repo (https://gitee.com/ftservo/FTServo_Python) but that version is
    not published to PyPI, so we monkeypatch here.
    """
...
class FeetechMotorsBus(SerialMotorsBus):
    """
    Efficient read/write to Feetech servos on a shared serial bus. Uses the
    `feetech-servo-sdk` PyPI package (imported as `scservo_sdk`).
    """
    ...
    def __init__(
        ...
        require_package("feetech-servo-sdk", import_name="scservo_sdk")
        super().__init__(port, motors, calibration)
        self.protocol_version = protocol_version
        self._assert_same_protocol()
        self.port_handler = scs.PortHandler(self.port)
        # Monkeypatch the buggy setPacketTimeout in the PyPI SDK.
    ...
    def configure_motors(self, return_delay_time=0, maximum_acceleration=254, acceleration=254) -> None:
        for motor in self.motors:
            # Reduce return-delay from default 500µs (250) to 2µs (0).
            self.write("Return_Delay_Time", motor, return_delay_time)
            # Bump max acceleration to speed up profile execution.
            if self.protocol_version == 0:
